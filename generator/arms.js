...
const endpoints =
{
    helmets: "https://finder.cstone.space/GetArmors/Arms"
}
...
async function Helmets() {
    ...
    try {
        ...
        console.log(" > Arms - data downloaded");
        for (const item of helmetData) {
            console.log(" > Arms - name:", item.Name);
            ids += " " + item.ItemId;

            html = "<table><tr><td colspan='2'><h3>" + item.Name + "</h3></td></tr><tr>";
			html += `<td><img src=/arms/` + item.ItemId + `.png></td><td><ul>`;
